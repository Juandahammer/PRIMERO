...
void Lectura1(int A[][12])
{
    ...
    scanf("%d %d %d", &MESm &PRO, &CAN);
    while (MES!= -1 && PRO!= -1 && CAN!=-1)
    {
        A[MES-1][PRO-1] += CAN;
        printf("Ingrese mes, tipo de producto y cantida vendida: ");
...
void Funcion1(int A[][12], int F, int C, float V1[], floatV2[])
{
    int I, J, SUM;
    printf("\n");
    for(I=0; I<F; I++)
...
void Funcion2(float A[], int C)
{
    ...
    for(I=0; I<C; I++)
        SUM += A[I];
    printf("\n\nTotal de ventas de la fabrica: %2.f", SUM);
}
